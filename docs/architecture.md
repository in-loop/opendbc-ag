...
What lives where in the repository, the data flow that produces the DBC corpus, and the CI gates that keep it consistent.
...
Each DBC file in `opendbc_ag/dbc/` is produced by a dedicated extractor in `opendbc_ag/tools/`. The extractor writes an intermediate JSON to `extractions/` first, then converts that JSON to DBC via `canmatrix`. The intermediate JSON is auditable and the DBC generation is reproducible.
...
Scope policy is enforced by code, not by review: anyone can open a PR; CI rejects scope violations. Cross-DBC duplicate PGN IDs are a build-time error rather than a runtime correctness bug.
