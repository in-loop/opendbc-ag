...
def scrape_list_page(page_num: int, session: requests.Session) -> tuple[list[VdmaPgnEntry], int | None]:
    ...
    # The PGN table rows contain links like /isobus/pGNAndSPN/<id>?type=PGN
    entries: list[VdmaPgnEntry] = []
    for a in soup.find_all("a", href=re.compile(r"/isobus/pGNAndSPN/\d+\?type=PGN")):
        # The row text typically contains "PGN_DECIMAL Name"
        text = a.get_text(strip=True)
        # Look for adjacent <td> elements to extract both PGN value and name
        ...
        if len(cells) < 2:
            continue
        # Standard layout: first cell PGN value, second cell name
        pgn_text = cells[0].get_text(strip=True)
        name_text = cells[1].get_text(strip=True) if len(cells) > 1 else ""
        try:
            # PGN can be decimal or hex with 0x prefix
    ...
    # Try to determine total pages from pagination if present
    total_pages = None
...
def main() -> int:
    ...
    status_md = repo / "RALPH_STATUS.md"
